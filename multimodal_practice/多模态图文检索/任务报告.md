...
本任务主要是关于多模态图文检索系统的设计与优化。采用的是双流编码器架构，结合ResNet101图像特征提取和BERT文本编码，通过的mean_max池化策略和困难负样本挖掘增强了跨模态匹配能力，还用了渐进式解冻训练和混合精度优化。该模型在Flickr8k数据集上将Mean R@1指标从初始的0.159提升至0.267，提升了67.9%。任务中还包含消融实验，验证了各项优化策略的有效性以及局限性
